...
internal object PlayerSubtitleRtlFix {
    ...
    fun fixTimedCues(
        ...
        for (entry in cues) {
            ...
            for (i in entry.cues.indices) {
                val original = entry.cues[i]
                val fixed = fixCueText(original, isBuiltInSubtitle)
                if (fixed !== original) {
                    if (modified == null) {
                        modified = ArrayList(entry.cues.size)
                        for (j in 0 until i) modified.add(entry.cues[j])
                    }
                    modified.add(fixed)
                } else {
    ...
    private fun appendMirroredReversed(out: Appendable, line: CharSequence, from: Int, toExclusive: Int) {
        ...
        while (i < toExclusive) {
            if (line[i].isDigit()) {
                ...
                while (i < toExclusive) {
                    if (line[i].isDigit()) {
                        i++
                    } else if (isNumberSeparator(line[i]) && i + 1 < toExclusive && line[i + 1].isDigit()) {
                        i++
                    } else break
                }
        ...
        for (idx in chunks.indices.reversed()) {
            ...
            if (range.last - range.first + 1 > 1) {
                ...
                val mirrored = mirrorPunctuation(c)
                if (mirrored != c) out.append(mirrored) else out.append(line.subSequence(range.first, range.first + 1))
            }
        }
    }
    ...
    private fun CharSequence.stripDirectionalWrap(): CharSequence {
        ...
        if (this !is Spanned) {
            ...
            for (ch in this) if (!isDirectionalMark(ch)) out.append(ch)
            return out.toString()
        }
        val out = SpannableStringBuilder(this)
        var index = 0
        while (index < out.length) {
            if (isDirectionalMark(out[index])) out.delete(index, index + 1) else index++
        }
        return out
    }
    ...
    private fun CharSequence.splitByNewlines(): List<CharSequence> {
        ...
        while (i < length) {
            if (this[i] == '\n') {
                result.add(subSequence(start, i))
                start = i + 1
            }
            i++
        }
    ...
    private fun containsRtlChars(text: CharSequence): Boolean {
        ...
        while (i < text.length) {
            val codePoint = Character.codePointAt(text, i)
            if (
                codePoint in 0x0590..0x05FF || codePoint in 0xFB1D..0xFB4F ||
                codePoint in 0x0600..0x06FF || codePoint in 0x0750..0x077F ||
                codePoint in 0x0870..0x08FF || codePoint in 0xFB50..0xFDFF ||
                codePoint in 0xFE70..0xFEFF
            ) return true
